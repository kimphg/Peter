...
{
	short azi;
	unsigned char dataI[FRAME_LEN];
	unsigned char dataQ[FRAME_LEN];
};
#define MAX_IREC 5000
#define OUTPUT_FRAME_SIZE 2051
...
void socketInit()
{
	...
	si_other.sin_port = htons(3456);//port "127.0.0.1"
...
void CalculateFFT(cufftComplex *h_signal, int fft_size)
{
	...
	while (true)
	{
		...
		while (iRead != iCur)
		{
			...
			for (int ir = 0; ir < FRAME_LEN; ir++)
			{
				...
				{
					h_signal[ir][i].x = dataBuff[ia].dataI[ir] - 127;
					h_signal[ir][i].y = dataBuff[ia].dataQ[ir] - 127;
					ia--;
					if (ia < 0)ia += MAX_IREC;
				}
			}
			...
			*/
			...
			// perform fft
			CalculateFFT((cufftComplex*)h_signal, FFT_SIZE);
			outputFrame[0] = u_char(dataBuff[iRead].azi >> 8);
			outputFrame[1] = u_char(dataBuff[iRead].azi);
			for (int i = 0; i < FRAME_LEN; i++)
			{
				...
				outputFrame[i + 2] = int(sqrt(double(maxAmp)) / double(FFT_SIZE) * 256);
			}
			//sendto(socket, outputFrame, strlen(message));
...
void packet_handler(u_char *param, const struct pcap_pkthdr *pkt_header, const u_char *pkt_data)
{
	...
	*/
	...
	if (((*(pkt_data + 36) << 8) | (*(pkt_data + 37))) != 2345)
	{
	...
	if (data[0] == 0)		//I chanel first part
	{
		...
		dataBuff[iRec].azi = (data[1] << 8) + (data[2]);
		memcpy(&dataBuff[iRec].dataI[0], data + 3, FRAME_LEN / 2);
	}
	else if (data[0] == 1) //Q chanel first part
	{
		memcpy(&dataBuff[iRec].dataQ[0], data + 3, FRAME_LEN / 2);
	}
	else if (data[0] == 2) //I chanel second part
	{
		memcpy(&dataBuff[iRec].dataI[0], data + 3 + FRAME_LEN / 2, FRAME_LEN / 2);
	}
	else if (data[0] == 3) //Q chanel second part
	{
		memcpy(&dataBuff[iRec].dataQ[0], data + 3 + FRAME_LEN / 2, FRAME_LEN / 2);
	}
	return;
	//    printf("len:%d\n", header->len);
