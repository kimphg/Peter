...
std::vector<std::vector<std::string> > Reader::readData(std::string fileloc){
    /*QFile file(QString::fromStdString(fileloc));
    ...
    while (!line.isNull())
    {
        ...
        foreach ( QString attl, attlist) {
            att.push_back(attl.toStdString());
        }
        data.push_back(att);
        line = in.readLine();
    }*/
    ...
    while(!fin.eof()){
        ...
        while (str) {
            ...
            if(temp.size())att.push_back(temp);
        }
        data.push_back(att);
    }
    return data;
