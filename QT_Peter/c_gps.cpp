...
unsigned long c_gps::parse_decimal()
{
  char *p = _term;
  bool isneg = *p == '-';
  if (isneg) ++p;
...
unsigned long c_gps::parse_degrees()
{
  char *p;
...
long c_gps::gpsatol(const char *str)
{
  long ret = 0;
...
int c_gps::gpsstrcmp(const char *str1, const char *str2)
{
...
void c_gps::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
...
void c_gps::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  if (date) *date = _date;
  if (time) *time = _time;
...
void c_gps::crack_datetime(int *year, unsigned char *month, unsigned char *day,
